...
2. **`distributed_gpu_queue.py`**: Manages GPU resources across multiple nodes and executes jobs in parallel
3. **`ERK_SNN_hparam_tuner.py`**: Performs hyperparameter optimization for a single experiment
4. **`ERK_SNN.py`**: Defines the ERK_SNN model architecture with ERK initialization
...
└────────┬────────────┘
         ...
         ▼
┌─────────────────────┐
│ distributed_gpu_    │  Manages GPU allocation
│ queue.py            │  Executes jobs in parallel
└────────┬────────────┘
         │
         ▼
┌─────────────────────┐
│ ERK_SNN_            │  Runs Optuna optimization
