#!/usr/bin/env python3
"""
Distributed GPU Queue Manager for NeST VNN Hyperparameter Tuning
...
Adapted from ERK_SNN scheduler with same logging style and format.

Usage:
    # On each node:
    python distributed_gpu_queue.py jobs/nest_vnn_advanced_jobs.json \
        --max-gpus 4 \
        --node-id node1 \
    ...
    L5: nohup python -u distributed_gpu_queue.py jobs/nest_vnn_advanced_jobs.json --max-gpus 19 --node-id l5 --lock-dir shared/locks --status-dir shared/status --log-file logs/distributed_queue_l5.log > logs/distributed_queue_l5.out 2>&1 &
    L4: nohup python distributed_gpu_queue.py jobs/nest_vnn_advanced_jobs.json --max-gpus 8 --node-id l4 --lock-dir shared/locks --status-dir shared/status --log-file logs/distributed_queue_l4.log > logs/distributed_queue_l4.out 2>&1 &
    L6: nohup python distributed_gpu_queue.py jobs/nest_vnn_advanced_jobs.json --max-gpus 4 --node-id l6 --lock-dir shared/locks --status-dir shared/status --log-file logs/distributed_queue_l6.log > logs/distributed_queue_l6.out 2>&1 &
"""
...
class DistributedGPUQueueManager:
    def __init__(self, max_gpus=None, log_file=None, node_id=None, 
        ...
        self.lock_dir = lock_dir or "shared/locks"
        self.status_dir = status_dir or "shared/status"
        ...
        # Get script directory for cd command
            ...
            if process.poll() is not None:
                ...
                if exit_code == 0:
                    ...
                    self._update_job_status(
                        ...
                        duration=duration,
                        node_id=self.node_id,
                        error=stderr.decode('utf-8')[:5000]  # First 2000 chars
                    )
...
def main():
    parser = argparse.ArgumentParser(
        description='Distributed GPU Queue Manager for NeST VNN Hyperparameter Tuning'
    )
