...
class TrainingDataWrapper():
	...
	def __init__(self, args):
		...
		# Load transcriptomic data instead of genomic features
		self.transcriptomic_data = np.genfromtxt(args.transcriptomic, delimiter = ',')
		# For transcriptomic data, we only have one feature per gene (expression level)
		# Reshape to match the expected format: (cells, genes, features)
		self.cell_features = self.transcriptomic_data.reshape(self.transcriptomic_data.shape[0], self.transcriptomic_data.shape[1], 1)
		...
		if hasattr(args, 'test') and args.test is not None:
			self.test_data = self.prepare_test_data(args.test)


	def prepare_train_data(self):
		return util.prepare_train_data(self.train, self.cell_id_mapping, self.zscore_method, self.std, self.seed)
	
	def prepare_test_data(self, test_file):
		"""
		Prepare test data for evaluation
		"""
		return util.prepare_predict_data(test_file, self.cell2id, self.zscore_method, self.std)
