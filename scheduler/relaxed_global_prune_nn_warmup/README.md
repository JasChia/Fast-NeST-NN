...
This creates:
- `jobs/global_prune_nn_jobs.txt` (600 jobs: 12 drugs × 50 experiments)
- `jobs/global_prune_nn_advanced_jobs.json` (same jobs with metadata)
...
nohup python advanced_gpu_queue.py jobs/global_prune_nn_advanced_jobs.json \
    --max-gpus 16 \
...
**On Node 1:**
```bash
nohup python distributed_gpu_queue.py jobs/global_prune_nn_advanced_jobs.json \
    --max-gpus 4 \
    --node-id l5 \
