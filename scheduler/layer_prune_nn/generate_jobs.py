#!/usr/bin/env python3
"""
Job Generator for Layer Pruned Fully Connected Neural Network (LayerPrunedFC_NN) Hyperparameter Tuning
Generates jobs files with drug experiments for the GPU queue manager, targeting
layer_prune_nn/layer_prune_nn_hparam_tuner.py and writing outputs under scheduler/layer_prune_nn/results/...

Should be run from the scheduler/layer_prune_nn directory.

Usage notes (run location):
- Run this script from the scheduler/layer_prune_nn directory so relative paths resolve correctly.
  Example:
...
def generate_sparse_jobs():
	"""Generate jobs.txt for LayerPrunedFC_NN hyperparameter tuner."""
	# Drug IDs
	...
	# Output file
	output_file = "jobs/layer_prune_nn_jobs.txt"
	...
	print(f"Generating LayerPrunedFC_NN hyperparameter tuning jobs for {len(drugs)} drugs...")
	...
	for drug in drugs:
		...
		for i in range(50):  # 50 experiments per drug
			...
			# Nested structure: layer_prune_nn/results/D{drug}/D{drug}_{i}
			...
			# Build the command for LayerPrunedFC_NN hyperparameter tuning
			command = (
	...
	with open(output_file, 'w') as f:
		f.write("# LayerPrunedFC_NN Hyperparameter Tuning Jobs\n")
...
def generate_sparse_jobs_json():
	"""Generate advanced_jobs.json file with LayerPrunedFC_NN hyperparameter tuning experiments."""
	# Drug IDs
	drugs = [5, 57, 80, 99, 127, 151, 188, 201, 244, 273, 298, 380]
	...
	print(f"Generating JSON jobs for LayerPrunedFC_NN hyperparameter tuning for {len(drugs)} drugs...")
	...
	for drug in drugs:
		...
		for i in range(50):  # 50 experiments per drug
			...
			job_config = {
				...
				"max_retries": 2,
				"description": f"Drug {drug} experiment {i} - LayerPrunedFC_NN Hyperparameter Tuning",
				"metadata": {
					"drug_id": drug,
					"experiment_id": i,
...
def main():
	"""Main function to generate LayerPrunedFC_NN job files (text and json)."""
	print("LayerPrunedFC_NN Hyperparameter Tuning Job Generator")
	print("=" * 60)

	print("\n1. Generating LayerPrunedFC_NN simple jobs.txt...")
	txt_file, txt_count = generate_sparse_jobs()

	print("\n2. Generating LayerPrunedFC_NN advanced jobs.json...")
	json_file, json_count = generate_sparse_jobs_json()

	print("\n" + "=" * 60)
	print("GENERATION COMPLETE")
	print("=" * 60)
	print(f"LayerPrunedFC_NN simple jobs file: {txt_file} ({txt_count} jobs)")
	print(f"LayerPrunedFC_NN advanced jobs file: {json_file} ({json_count} jobs)")
	print(f"Total LayerPrunedFC_NN jobs generated: {txt_count}")

	print("\nUsage:")
	print(f"python advanced_gpu_queue.py jobs/layer_prune_nn_advanced_jobs.json --max-gpus 16")
	print(f"tail -f layer_prune_nn/logs/gpu_queue_manager.log")
