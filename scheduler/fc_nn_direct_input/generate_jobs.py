#!/usr/bin/env python3
"""
Job Generator for Fully Connected Neural Network with Residual Connections (FC_NN_Residual) Hyperparameter Tuning
Generates jobs files with drug experiments for the GPU queue manager, targeting
fc_nn_residual/fc_nn_residual_hparam_tuner.py and writing outputs under scheduler/fc_nn_residual/results/...

Should be run from the scheduler/fc_nn_residual directory.

Usage notes (run location):
- Run this script from the scheduler/fc_nn_residual directory so relative paths resolve correctly.
  Example:
    cd /nfs/ml_lab/projects/Pilot1_PreclinicalHPC/jchia_backup/jchia_l05_7-31-25/scheduler/fc_nn_residual
...
def generate_sparse_jobs():
	"""Generate jobs.txt for FC_NN_Residual hyperparameter tuner."""
	# Drug IDs
	...
	# Output file
	output_file = "jobs/fc_nn_residual_jobs.txt"
	...
	print(f"Generating FC_NN_Residual hyperparameter tuning jobs for {len(drugs)} drugs...")
	...
	for drug in drugs:
		...
		for i in range(50):  # 50 experiments per drug
			...
			# Build the command for FC_NN_Residual hyperparameter tuning
			command = (
				f"python -u fc_nn_residual_hparam_tuner.py "
				f"-drug {drug} "
	...
	with open(output_file, 'w') as f:
		f.write("# FC_NN_Residual Hyperparameter Tuning Jobs\n")
...
def generate_sparse_jobs_json():
	"""Generate advanced_jobs.json file with FC_NN_Residual hyperparameter tuning experiments."""
	# Drug IDs
	drugs = [5, 57, 80, 99, 127, 151, 188, 201, 244, 273, 298, 380]
	...
	# Output file
	output_file = "jobs/fc_nn_residual_advanced_jobs.json"
	...
	print(f"Generating JSON jobs for FC_NN_Residual hyperparameter tuning for {len(drugs)} drugs...")
	...
	for drug in drugs:
		...
		for i in range(50):  # 50 experiments per drug
			...
			job_config = {
				...
				"max_retries": 2,
				"description": f"Drug {drug} experiment {i} - FC_NN_Residual Hyperparameter Tuning",
				"metadata": {
					"drug_id": drug,
					"experiment_id": i,
	...
	json_data = {
		"description": "FC_NN_Residual Hyperparameter Tuning Jobs",
...
def main():
	"""Main function to generate FC_NN_Residual job files (text and json)."""
	print("FC_NN_Residual Hyperparameter Tuning Job Generator")
	print("=" * 60)

	print("\n1. Generating FC_NN_Residual simple jobs.txt...")
	txt_file, txt_count = generate_sparse_jobs()

	print("\n2. Generating FC_NN_Residual advanced jobs.json...")
	json_file, json_count = generate_sparse_jobs_json()

	print("\n" + "=" * 60)
	print("GENERATION COMPLETE")
	print("=" * 60)
	print(f"FC_NN_Residual simple jobs file: {txt_file} ({txt_count} jobs)")
	print(f"FC_NN_Residual advanced jobs file: {json_file} ({json_count} jobs)")
	print(f"Total FC_NN_Residual jobs generated: {txt_count}")

	print("\nUsage:")
	print(f"python advanced_gpu_queue.py jobs/fc_nn_residual_advanced_jobs.json --max-gpus 16")
	print(f"tail -f fc_nn_residual/logs/gpu_queue_manager.log")
