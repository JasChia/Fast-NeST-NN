"""Hyperparameter tuning for Uniform Random Direct Output Direct Input Sparse Neural Network.

This module performs hyperparameter optimization for a sparse neural network that uses
uniform random pruning for initialization and includes:
1. Direct Output connections: nodes in each intermediary layer are split into non-overlapping 
   sets that predict the final output.
...
from uniform_random_do_di_snn import UniformRandomDODISNN
...
class OptunaDODISparseNNTrainer:
    """Hyperparameter tuning trainer for Uniform Random DO+DI Sparse NN using Optuna."""
    ...
    def exec_study(self) -> Dict[str, float]:
        ...
        storage = optuna.storages.JournalStorage(
            optuna.storages.JournalFileStorage(str(self.save_dir / "UniformRandomDODISNN_HTune.log"))
        )
        ...
        study = optuna.create_study(
            study_name="UniformRandomDODISNN_HTune",
            storage=storage,
            direction="maximize",
            sampler=sampler,
        ...
        try:
            ...
            for epoch in range(self.epochs):
                ...
                for input_data, labels in dataloader:
                    ...
                    # Total loss: main loss + L1 regularization
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train Uniform Random DO+DI (Direct Output + Direct Input) Sparse Neural Network")
...
def main() -> None:
    """Main entry point."""
    args = parse_args()
    hparam_trainer = OptunaDODISparseNNTrainer(args)
