"""Hyperparameter tuning for eNest_sum (eNest with sum/linear output methods)."""
...
class OptunaeNestTrainer:
    """Hyperparameter tuning trainer for eNest_sum using Optuna."""
    ...
    def exec_study(self) -> Dict[str, float]:
        ...
        storage = optuna.storages.JournalStorage(
            optuna.storages.JournalFileStorage(str(self.save_dir / "eNest_sum_HTune.log"))
        )
    ...
    def setup_trials(self, trial: Trial) -> None:
        ...
        # NEW: BatchNorm position selection
        self.batchnorm_position = trial.suggest_categorical("batchnorm_position", ["before_activation", "after_activation"])
        ...
        print(f"Trial {trial.number}: h={self.genotype_hiddens}, lr={self.lr:.2e}, drop={self.dropout:.2f}, "
              ...
              f"output={self.output_method}, bn_pos={self.batchnorm_position}", end="")
        sys.stdout.flush()
        ...
        try:
            ...
            try:
                model = eNest(
                    ...
                    batchnorm_position=self.batchnorm_position,
                    verbosity=-1
                ).to(self.device)
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train eNest_sum (eNest with sum/linear output methods)")
