...
global_prune_nn/global_prune_nn_hparam_tuner.py and writing outputs under scheduler/global_prune_nn/results/...

Should be run from the scheduler/global_prune_nn directory.

Usage notes (run location):
- Run this script from the scheduler/global_prune_nn directory so relative paths resolve correctly.
  Example:
    cd /nfs/ml_lab/projects/Pilot1_PreclinicalHPC/jchia_backup/jchia_l05_7-31-25/scheduler/global_prune_nn
...
def generate_sparse_jobs():
	...
	# Output file
	output_file = "jobs/global_prune_nn_jobs.txt"
	...
	with open(output_file, 'w') as f:
		f.write("# GlobalPrunedFC_NN Hyperparameter Tuning Jobs\n")
...
def main():
	...
	print("\nUsage:")
	print(f"python advanced_gpu_queue.py jobs/global_prune_nn_advanced_jobs.json --max-gpus 16")
	print(f"tail -f global_prune_nn/logs/gpu_queue_manager.log")
