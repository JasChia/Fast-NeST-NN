#!/usr/bin/env python3
"""
Job Generator for Uniform Random Direct Output Sparse Neural Network Hyperparameter Tuning
Generates jobs files with drug experiments for the GPU queue manager, targeting
uniform_random_snn_hparam_tuner.py and writing outputs under results/...

Should be run from the scheduler/uniform_random_direct_output_SNN directory.

Usage notes (run location):
- Run this script from the scheduler/uniform_random_direct_output_SNN directory so relative paths resolve correctly.
  Example:
    cd /nfs/ml_lab/projects/Pilot1_PreclinicalHPC/jchia_backup/jchia_l05_7-31-25/scheduler/uniform_random_direct_output_SNN
...
def generate_sparse_jobs():
	"""Generate jobs.txt for Uniform Random Direct Output Sparse NN hyperparameter tuner."""
	# Drug IDs
	...
	# Output file
	output_file = "jobs/uniform_random_snn_jobs.txt"
	...
	print(f"Generating Uniform Random Direct Output Sparse NN hyperparameter tuning jobs for {len(drugs)} drugs...")
	...
	for drug in drugs:
		...
		for i in range(50):  # 50 experiments per drug
			...
			# Build the command for Uniform Random Direct Output Sparse NN hyperparameter tuning
			command = (
				f"python -u uniform_random_snn_hparam_tuner.py "
				f"-drug {drug} "
	...
	with open(output_file, 'w') as f:
		f.write("# Uniform Random Direct Output Sparse NN Hyperparameter Tuning Jobs\n")
...
def generate_sparse_jobs_json():
	"""Generate advanced_jobs.json file with Uniform Random Direct Output Sparse NN hyperparameter tuning experiments."""
	# Drug IDs
	drugs = [5, 57, 80, 99, 127, 151, 188, 201, 244, 273, 298, 380]
	...
	# Output file
	output_file = "jobs/uniform_random_snn_advanced_jobs.json"
	...
	print(f"Generating JSON jobs for Uniform Random Direct Output Sparse NN hyperparameter tuning for {len(drugs)} drugs...")
	...
	for drug in drugs:
		...
		for i in range(50):  # 50 experiments per drug
			...
			job_config = {
				...
				"max_retries": 2,
				"description": f"Drug {drug} experiment {i} - Uniform Random Direct Output Sparse NN Hyperparameter Tuning",
				"metadata": {
					"drug_id": drug,
					"experiment_id": i,
	...
	json_data = {
		"description": "Uniform Random Direct Output Sparse NN Hyperparameter Tuning Jobs",
...
def main():
	"""Main function to generate Uniform Random Direct Output Sparse NN job files (text and json)."""
	print("Uniform Random Direct Output Sparse NN Hyperparameter Tuning Job Generator")
	print("=" * 60)
	...
	print("\nUsage:")
	print(f"python distributed_gpu_queue.py jobs/uniform_random_snn_advanced_jobs.json --max-gpus 16 --node-id node1 --lock-dir shared/locks --status-dir shared/status")
