"""Hyperparameter tuning for Uniform Random Direct Output Sparse Neural Network.

This module performs hyperparameter optimization for a sparse neural network that uses
uniform random pruning for initialization and includes direct output connections where nodes
in each intermediary layer are split into non-overlapping sets that predict the final output.
"""
...
#Data loading relic from NeST_VNN code base https://github.com/idekerlab/nest_vnn
...
#Evaluation and saving metrics code
def evaluate_model_metrics(
    ...
    model.eval()
    with torch.no_grad():
        predictions, _ = model(data.to(device), return_auxiliary=False)
...
class OptunaSparseNNTrainer:
    """Hyperparameter tuning trainer for Uniform Random Direct Output Sparse NN using Optuna."""
    ...
    def exec_study(self) -> Dict[str, float]:
        ...
        storage = optuna.storages.JournalStorage(
            optuna.storages.JournalFileStorage(str(self.save_dir / "UniformRandomSNN_HTune.log"))
        )
        ...
        study = optuna.create_study(
            study_name="UniformRandomSNN_HTune",
            storage=storage,
            direction="maximize",
            sampler=sampler,
    ...
    def setup_trials(self, trial: Trial) -> None:
        ...
        # Alpha parameter for auxiliary loss weighting (0.0 to 1.0)
        self.alpha = trial.suggest_float("alpha", 0.0, 1.0, log=False, step=0.1)
        ...
        print(f"Trial {trial.number}: h={self.genotype_hiddens}, lr={self.lr:.2e}, drop={self.dropout:.2f}, alpha={self.alpha:.2f}, act={activation_choice}, batch_size={int(2 ** self.batch_size_power)}", end="")
        sys.stdout.flush()
    ...
    def train_model(self, trial: Trial) -> float:
        ...
        try:
            ...
            try:
                ...
                print(f"Warning: Trial {trial.number} failed to generate a satisfying sparse network architecture.")
                print(f"  This may occur when the sparse network generation cannot satisfy the connectivity constraints.")
                print(f"  Error details: {str(e)}")
                print(f"  Skipping this trial and continuing with the next one.")
                raise e

            optimizer = torch.optim.AdamW(model.parameters(), lr=self.lr, betas=(0.9, 0.99), eps=1e-5, weight_decay=self.wd)
            loss_fn = nn.MSELoss()
            ...
            for epoch in range(self.epochs):
                ...
                for input_data, labels in dataloader:
                    ...
                    main_pred, auxiliary_outputs = model(input_data, return_auxiliary=True)

                    # Main loss
                    mse_loss = loss_fn(main_pred, labels)
                    
                    # Auxiliary losses: each intermediary layer's predictions vs final output
                    auxiliary_loss = torch.tensor(0.0, device=self.device)
                    if self.alpha > 0.0 and len(auxiliary_outputs) > 0:
                        for aux_pred in auxiliary_outputs:
                            # aux_pred shape: (batch_size, num_groups)
                            # We need to compare each group's prediction to the true label
                            # Expand labels to match: (batch_size, 1) -> (batch_size, num_groups)
                            num_groups = aux_pred.shape[1]
                            labels_expanded = labels.expand(-1, num_groups)  # (batch_size, num_groups)
                            # Compute MSE for each group and average
                            aux_mse = loss_fn(aux_pred, labels_expanded)
                            auxiliary_loss = auxiliary_loss + aux_mse
                        # Average across all intermediary layers
                        auxiliary_loss = auxiliary_loss / len(auxiliary_outputs)
                    
                    l1_loss = sum(p.abs().sum() for p in model.parameters())

                    # Total loss: main loss + alpha * auxiliary loss + L1 regularization
                    total_loss = mse_loss + self.alpha * auxiliary_loss + l1_loss * self.l1
                ...
                with torch.no_grad():
                    model.eval()
                    cl_val_pred, _ = model(cl_val_data, return_auxiliary=False)
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train Uniform Random Direct Output Sparse Neural Network")
...
def main() -> None:
    """Main entry point."""
    args = parse_args()
    hparam_trainer = OptunaSparseNNTrainer(args)
