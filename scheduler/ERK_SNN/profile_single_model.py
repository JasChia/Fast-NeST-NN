#!/usr/bin/env python3
"""
Utility script to train a single ERK_SNN model and emit timing information for
the expensive sections (data loading, sparse mask generation, each epoch, etc.).

Example (from this directory, with data under ``<repo>/Data``):
...
def main():
    parser = argparse.ArgumentParser(description="Profile a single ERK_SNN training run")
    ...
    print("===== Profiling Single ERK_SNN Run =====")
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            model.eval()
            val_preds = model(val_input.to(device))
