"""
Compute sparsity level of each best model in global_prune_nn_warmup/results.
...
RESULTS_DIR = "/nfs/ml_lab/projects/Pilot1_PreclinicalHPC/jchia_backup/jchia_l05_7-31-25/scheduler/global_prune_nn_warmup/results"
...
def extract_nn_index(key):
    """Extract the numeric index from a key like 'NN.4.weight_orig' -> 4"""
...
def compute_sparsity_from_state_dict(state_dict):
    ...
    """
    # Find all unique NN layer indices that have a 2D weight parameter
    layer_weights = {}
    ...
    for key in state_dict.keys():
        ...
        elif key.endswith('.weight') and f'NN.{idx}.weight_orig' not in state_dict:
            tensor = state_dict[key]
            if tensor.dim() == 2:
                layer_weights[idx] = {
                    'shape': tuple(tensor.shape),
    ...
    if len(sorted_indices) < 9:
        raise ValueError(
            "Expected at least 9 linear layers (8 hidden + 1 output), "
            "got {}. Indices: {}".format(len(sorted_indices), sorted_indices)
        )
    
    # Last linear layer is output; the rest are hidden
...
def main():
    ...
    print("\n=== DETAILED LAYER BREAKDOWN (first model) ===")
    first_drug = drugs[0]
    first_exp = sorted(
