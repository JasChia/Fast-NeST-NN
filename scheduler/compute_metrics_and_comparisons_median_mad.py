...
Also generates tables performance tables (median ± MAD) and p-values (paired t-tests and Wilcoxon rank-sum tests with and without BH column-wise adjustment).

This version uses median and mean absolute deviation (MAD) instead of mean and standard deviation.
"""
...
def compute_mad(values):
    """
    Compute mean absolute deviation (MAD) from the median.
    MAD = mean(|x_i - median(x)|)
    """
    if len(values) == 0:
        return np.nan
...
def compute_paired_tests(df, baseline_method='eNest_linear_fair', metric='r2_test'):
    ...
    Returns:
    - Dictionary: {drug_id -> {method -> {'median': float, 'mad': float, 'p_value': float}}}
    """
    ...
    for drug_id in drugs:
                ...
                if len(method_data) > 0:
                    ...
                    if len(method_values) > 0:
                        results[drug_id][method] = {
                            'median': np.median(method_values),
                            'mad': compute_mad(method_values),
        ...
        if len(baseline_values) > 0:
            results[drug_id][baseline_method] = {
                ...
                'mad': compute_mad(baseline_values),
                'p_value': np.nan
            }
        ...
        for method in methods:
            ...
            # Calculate median and MAD from all method data (not just paired)
            method_values = method_data[metric].dropna()
            if len(method_values) > 0:
                method_median = np.median(method_values)
                method_mad = compute_mad(method_values)
            else:
...
def create_comparison_table(results, methods, drugs, baseline_method='eNest_linear_fair', use_bh_adjustment=False):
    """
    Create a formatted comparison table using median ± MAD.
    ...
    for drug_id in drugs:
        ...
        for method in methods:
            if drug_id in results and method in results[drug_id]:
                ...
                median_val = method_result['median']
                mad_val = method_result['mad']
                p_val = method_result['p_value']
                
                if pd.notna(median_val) and pd.notna(mad_val):
                    value_str = f'{median_val:.4f} ± {mad_val:.4f}'
...
def main():
    ...
    output_dir = base_dir / 'metrics_analysis' / 'median_mad'
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # Check if combined CSV already exists (use the main one from parent directory)
    combined_csv_path = base_dir / 'metrics_analysis' / 'combined_metrics_all_methods.csv'
    ...
    if combined_csv_path.exists():
        ...
        # Write combined CSV (to parent directory, shared with main script)
        combined_csv_path.parent.mkdir(exist_ok=True)
    ...
    print("\n" + "=" * 60)
    print(f"CREATING R² TEST COMPARISON TABLES (vs {baseline_method}) - MEDIAN ± MAD")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print(f"CREATING PEARSON TEST COMPARISON TABLES (vs {baseline_method}) - MEDIAN ± MAD")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print(f"CREATING SPEARMAN TEST COMPARISON TABLES (vs {baseline_method}) - MEDIAN ± MAD")
    print("=" * 60)
    ...
    print(f"\nFiles created:")
    print(f"  1. {ttest_unadjusted_path.name} (R² test paired t-test unadjusted p-values)")
    print(f"  2. {ttest_adjusted_path.name} (R² test paired t-test BH column-wise adjusted p-values)")
    print(f"  3. {wilcoxon_unadjusted_path.name} (R² test Wilcoxon rank-sum unadjusted p-values)")
    print(f"  4. {wilcoxon_adjusted_path.name} (R² test Wilcoxon rank-sum BH column-wise adjusted p-values)")
    print(f"  5. {pearson_ttest_unadjusted_path.name} (Pearson test paired t-test unadjusted p-values)")
    print(f"  6. {pearson_ttest_adjusted_path.name} (Pearson test paired t-test BH column-wise adjusted p-values)")
    print(f"  7. {pearson_wilcoxon_unadjusted_path.name} (Pearson test Wilcoxon rank-sum unadjusted p-values)")
    print(f"  8. {pearson_wilcoxon_adjusted_path.name} (Pearson test Wilcoxon rank-sum BH column-wise adjusted p-values)")
    print(f"  9. {spearman_ttest_unadjusted_path.name} (Spearman test paired t-test unadjusted p-values)")
    print(f" 10. {spearman_ttest_adjusted_path.name} (Spearman test paired t-test BH column-wise adjusted p-values)")
    print(f" 11. {spearman_wilcoxon_unadjusted_path.name} (Spearman test Wilcoxon rank-sum unadjusted p-values)")
    print(f" 12. {spearman_wilcoxon_adjusted_path.name} (Spearman test Wilcoxon rank-sum BH column-wise adjusted p-values)")
