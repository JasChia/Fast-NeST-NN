        ...
        try:
            ...
            for epoch in range(epochs):
                ...
                with torch.no_grad():
                    ...
                    # Save best model based on validation loss, but only if fully pruned to target
                    if model.is_fully_pruned_to_target():
                        if min_loss is None:
                            ...
                            print(f"Model saved at epoch {epoch} (fully pruned to target)")
                    # else:
                    #     # Update best loss tracking even if not saving (for early stopping)
                    #     if min_loss is None:
                    #         min_loss = new_val_loss
                    #     elif min_loss - new_val_loss > 0.0001:
                    #         min_loss = new_val_loss
            ...
            if best_model_state is not None:
                ...
                print("Removing pruning masks from model...")
                ...
                print("Best model saved as model_final.pt (pruning masks removed)")
            elif model.is_fully_pruned_to_target():
                print("\nNo best model found, using final model...")
                # Remove pruning masks from final model
                print("Removing pruning masks from model...")
                for layer in model.NN:
                    if isinstance(layer, torch.nn.Linear):
                        if hasattr(layer, 'weight_mask'):
                            prune.remove(layer, 'weight')
                torch.save(deepcopy(model.state_dict()), f"{trial_dir}/model_final.pt")
                print("Final model saved as model_final.pt (pruning masks removed)")
            else:
