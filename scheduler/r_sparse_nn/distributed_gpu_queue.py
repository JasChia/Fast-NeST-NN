...
Usage:
    # On each node:
    python distributed_gpu_queue.py jobs/r_sparse_nn_advanced_jobs.json \
        --max-gpus 4 \
        --node-id node1 \
    ...
    nohup python distributed_gpu_queue.py jobs/r_sparse_nn_advanced_jobs.json --max-gpus 8 --node-id l4 --lock-dir shared/locks --status-dir shared/status --log-file logs/distributed_queue_l4.log > logs/distributed_queue_l4.out 2>&1 &
"""
...
class DistributedGPUQueueManager:
    ...
    def start_job(self, job_info, gpu_id):
        ...
        try:
            ...
            conda_command = (
                f"cd /nfs/ml_lab/projects/Pilot1_PreclinicalHPC/jchia_backup/jchia_l05_7-31-25/scheduler/r_sparse_nn && "
