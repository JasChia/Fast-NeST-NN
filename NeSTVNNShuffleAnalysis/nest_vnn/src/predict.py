...
parser.add_argument('-transcriptomic', help = 'Transcriptomic data (gene expression) for cell lines', type = str)
...
transcriptomic_data = np.genfromtxt(opt.transcriptomic, delimiter = ',')
# Reshape to match the expected format: (cells, genes, features)
cell_features = transcriptomic_data.reshape(transcriptomic_data.shape[0], transcriptomic_data.shape[1], 1)
