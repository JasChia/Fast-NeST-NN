# Hyperparameter tuning for Dense_fNeST (dense fNeST with direct input and layer predictions).
# Lineage: scheduler/fc_nn_DI_Layer_Pred (fc_nn_di_layer_pred_hparam_tuner); jobs from scripts/build_ape_jobs_from_scheduler.py.
...
import pandas as pd
import torch
from dense_fnest_fc_nn import Dense_fNeST
...
parser = argparse.ArgumentParser(description = 'Train Dense_fNeST (fully connected fast NeST-style network with direct input and layer predictions)')
...
class OptunaDenseFNeSTTrainer():
    ...
    def exec_study(self):
        storage = optuna.storages.JournalStorage(
            optuna.storages.JournalFileStorage(f"{self.save_dir}/Dense_fNeST_HTune.log")
        )
        median_pruner = optuna.pruners.MedianPruner(
            n_startup_trials=15, n_warmup_steps=0, interval_steps=1
        )
        study_name = f"Dense_fNeST_HTune"
    ...
    def setup_trials(self, trial):
        # Hyperparameter suggestions for Dense_fNeST
    ...
    def train_model(self, trial):
        ...
        try:
            ...
            # Create Dense_fNeST model with hyperparameters
            ...
            for epoch in range(epochs):
                for i, (input_data, labels) in enumerate(cl_dataloader):
                    ...
                    # Dense_fNeST returns single aggregated output
...
if __name__ == "__main__":
    hparam_trainer = OptunaDenseFNeSTTrainer()
