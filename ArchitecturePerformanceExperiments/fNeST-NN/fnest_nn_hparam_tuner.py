...
Lineage: same training stack as ``scheduler/eNest_linear_fair`` (``eNest_hparam_tuner`` / ``eNest``);
bundled jobs are generated from that tree via ``scripts/build_ape_jobs_from_scheduler.py``.
"""
...
def main() -> None:
    """Main entry point."""
    args = parse_args()
    hparam_trainer = OptunaFNeSTNNTrainer(args)
