...
_SCHED = Path(__file__).resolve().parent.parent
if str(_SCHED) not in sys.path:
    sys.path.insert(0, str(_SCHED))
from data_paths import data_root_from  # noqa: E402

_DATA_ROOT = str(data_root_from(__file__))


class eNest(nn.Module):
    def __init__(self, nodes_per_assembly, dropout=0.0, activation=nn.Tanh, 
                 output_method='final', verbosity=-1):
        """
        eNest model with configurable output method.
