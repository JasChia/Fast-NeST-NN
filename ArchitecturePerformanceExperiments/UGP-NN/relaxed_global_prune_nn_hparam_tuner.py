...
from relaxed_global_prune_fc_nn import RelaxedGlobalPrunedFC_NN
...
class OptunaRelaxedGlobalPruneNNTrainer():
    ...
    def exec_study(self):
        storage = optuna.storages.JournalStorage(
            optuna.storages.JournalFileStorage(f"{self.save_dir}/RelaxedGlobalPrunedFC_NN_HTune.log")
        )
        median_pruner = optuna.pruners.MedianPruner(
            n_startup_trials=15, n_warmup_steps=0, interval_steps=1
        )
        study_name = f"RelaxedGlobalPrunedFC_NN_HTune"
    ...
    def setup_trials(self, trial):
        ...
        self.prune_percentage = trial.suggest_float("prune_percentage", 0.01, 0.5, log=False, step=0.01)  # Percentage to prune (0.0 to 0.5)
        
        # Warmup hyperparameter - number of epochs to train without pruning
        self.warmup = trial.suggest_int("warmup", 0, 200, step=1)  # Number of epochs before starting pruning
    ...
    def train_model(self, trial):
        ...
        try:
            ...
            # Create RelaxedGlobalPrunedFC_NN model with hyperparameters
            # Use trial number to create unique seed for each trial while maintaining reproducibility
            trial_seed = trial.number + args.seed
            # Model creation
            
            try:
                model = RelaxedGlobalPrunedFC_NN(
                    input_dim=689,  # Fixed input dimension as required by RelaxedGlobalPrunedFC_NN
            ...
            for epoch in range(epochs):
                ...
                with torch.no_grad():
                    ...
                    # but only after warmup + at least one prune step has elapsed.
                    # First qualifying epoch always saves; subsequent epochs save only on a strict improvement.
                    if (epoch + 1) > (self.warmup + self.prune_frequency):
            ...
            if has_best:
                ...
                print("\nNo best checkpoint recorded; using final model state...")
...
if __name__ == "__main__":
    hparam_trainer = OptunaRelaxedGlobalPruneNNTrainer()
