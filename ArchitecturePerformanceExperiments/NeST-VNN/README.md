...
**Shared conventions:** **[`../README.md`](../README.md)** (data, jobs). The **gene-order shuffle** pipeline is separate: **`NeSTVNNShuffleAnalysis/`** at repo root.
...
```bash
cd ArchitecturePerformanceExperiments/NeST-VNN
...
Use the Conda **`environment.yml`** at the repository root (see root **`README.md`**; `conda activate fast-nest-nn`) with PyTorch, Optuna, pandas, **`torchmetrics`**, and the NeST-VNN **`src/`** dependencies.
