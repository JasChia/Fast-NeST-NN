...
from layer_prune_fc_nn import LayerPrunedFC_NN
...
class OptunaLayerPruneNNTrainer():
    ...
    def exec_study(self):
        storage = optuna.storages.JournalStorage(
            optuna.storages.JournalFileStorage(f"{self.save_dir}/LayerPrunedFC_NN_HTune.log")
        )
        median_pruner = optuna.pruners.MedianPruner(
            n_startup_trials=15, n_warmup_steps=0, interval_steps=1
        )
        study_name = f"LayerPrunedFC_NN_HTune"
    ...
    def setup_trials(self, trial):
        ...
        self.prune_frequency = trial.suggest_int("prune_frequency", 1, 50, step=1)  # How often to prune (every N epochs)
        ...
        print(f"Trial {trial.number}: h={self.genotype_hiddens}, lr={self.lr:.2e}, drop={self.dropout:.2f}, act={activation_choice}, batch_size={int(2 ** self.batch_size_power)}, prune_freq={self.prune_frequency}, prune_pct={self.prune_percentage:.3f}, batchnorm={self.use_batchnorm}")
        sys.stdout.flush()
    ...
    def train_model(self, trial):
        ...
        try:
            ...
            # Create LayerPrunedFC_NN model with hyperparameters
            # Use trial number to create unique seed for each trial while maintaining reproducibility
            trial_seed = trial.number + args.seed
            # Model creation
            
            try:
                model = LayerPrunedFC_NN(
                    input_dim=689,  # Fixed input dimension as required by LayerPrunedFC_NN
            ...
            for epoch in range(epochs):
                ...
                # Prune model every prune_frequency epochs
                if self.prune_percentage > 0.0 and (epoch + 1) % self.prune_frequency == 0:
                    model.train()  # Set to train mode for pruning
                    model.prune(sparsity_level=self.prune_percentage)
                    print(f"Pruned model at epoch {epoch + 1} with {self.prune_percentage:.3f} sparsity level")
                    sys.stdout.flush()
...
if __name__ == "__main__":
    hparam_trainer = OptunaLayerPruneNNTrainer()
