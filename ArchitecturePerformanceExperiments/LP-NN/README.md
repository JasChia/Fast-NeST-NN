...
**Note:** Long sections below (system architecture, `generate_jobs.py`, GPU queue helpers) are **historical**. This repo ships **`jobs/*.txt`** / **`jobs/*.json`** and `*_hparam_tuner.py` only—see **Running bundled jobs** near the top.
...
In-tree `generate_jobs.py` and GPU queue scripts are **not** shipped here. Use **`jobs/layer_prune_nn_jobs.txt`** from **`ArchitecturePerformanceExperiments/LP-NN/`** (one command per line; skip `#` comments). Paths assume **`Data/`** at the repo root (`../../Data/...`). Execute with Slurm, GNU Parallel, or your scheduler.
...
- **Bundled job lists**: `jobs/layer_prune_nn_jobs.txt` for batch execution on your cluster
...
**Key Components**:
- `OptunaLayerPruneNNTrainer` class: Manages hyperparameter tuning
...
- Drug ID (10 drugs: 5, 80, 99, 127, 151, 188, 244, 273, 298, 380)
...
Bundled commands are in **`jobs/layer_prune_nn_jobs.txt`** and **`jobs/layer_prune_nn_advanced_jobs.json`**. From **`ArchitecturePerformanceExperiments/LP-NN/`**:

```bash
cd ArchitecturePerformanceExperiments/LP-NN
grep -v '^#' jobs/layer_prune_nn_jobs.txt | grep -v '^$' | bash
```

Use your cluster scheduler for parallel GPU runs. See **“Running bundled jobs”** at the top of this README.
...
Run training commands from **`jobs/layer_prune_nn_jobs.txt`** (each line invokes **`layer_prune_nn_hparam_tuner.py`** with paths under **`../../Data/`**).
