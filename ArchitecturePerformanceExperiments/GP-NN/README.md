...
In-tree `generate_jobs.py` and GPU queue scripts are **not** shipped here. Use **`jobs/global_prune_nn_warmup_jobs.txt`** (one command per line; skip `#` comments) from **`ArchitecturePerformanceExperiments/GP-NN/`**. Paths assume **`Data/`** at the repo root (`../../Data/...`). Execute lines with Slurm, GNU Parallel, or your site’s job runner.
...
- **Bundled job lists**: `jobs/global_prune_nn_warmup_jobs.txt` for batch execution on your cluster
...
**Key Components**:
- `OptunaGlobalPruneNNTrainer` class: Manages hyperparameter tuning
...
Bundled commands are in **`jobs/global_prune_nn_warmup_jobs.txt`** and **`jobs/global_prune_nn_warmup_advanced_jobs.json`** (paths use **`../../Data/`**). From **`ArchitecturePerformanceExperiments/GP-NN/`**:

```bash
cd ArchitecturePerformanceExperiments/GP-NN
grep -v '^#' jobs/global_prune_nn_warmup_jobs.txt | grep -v '^$' | bash
```

Use your cluster scheduler (Slurm, GNU Parallel, etc.) for parallel GPU runs. See also the short **“Running bundled jobs”** section at the top of this README.
