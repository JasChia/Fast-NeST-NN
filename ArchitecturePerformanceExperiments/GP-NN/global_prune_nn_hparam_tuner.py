# Hyperparameter tuning for Sparse_NN
# python sparse_nn_hparam_tuner.py > sparse_nn_hparam.log 2>&1 &
# nohup python -u sparse_nn_hparam_tuner.py -cuda 0 -drug 188 > Shuffle_Input/CombatLog2TPM/D188/HTune_logs.txt 2>&1 &
# nohup python -u sparse_nn_hparam_tuner.py -cuda 3 -drug 143 > eNest_New_CL/Log2TPM/D143/HTune_logs.txt 2>&1 &
...
from global_prune_fc_nn import GlobalPrunedFC_NN
...
def evaluate_model_metrics(model, data, labels, device):
    ...
    with torch.no_grad():
        ...
        # Calculate metrics
        pearson_corr = pearson_correlation(predictions, labels_dev)
        spearman_corr = spearman_correlation(predictions, labels_dev)
        r2 = r2_score(labels_dev.cpu().numpy().flatten(), predictions.cpu().numpy().flatten())
...
parser = argparse.ArgumentParser(description = 'Train Sparse_NN')
...
class OptunaGlobalPruneNNTrainer():
    ...
    def exec_study(self):
        storage = optuna.storages.JournalStorage(
            optuna.storages.JournalFileStorage(f"{self.save_dir}/GlobalPrunedFC_NN_HTune.log")
        )
        median_pruner = optuna.pruners.MedianPruner(
            n_startup_trials=15, n_warmup_steps=0, interval_steps=1
        )
        study_name = f"GlobalPrunedFC_NN_HTune"
    ...
    def setup_trials(self, trial):
        # Hyperparameter suggestions for Sparse_NN
        ...
        # max_attempts uses default value (10000) for sparse network generation
        ...
        self.prune_frequency = trial.suggest_int("prune_frequency", 1, 10, step=1)  # How often to prune (every N epochs)
        self.prune_percentage = trial.suggest_float("prune_percentage", 0.05, 0.5, log=False, step=0.05)  # Percentage to prune (0.0 to 0.5)
        
        # Warmup hyperparameter - number of epochs to train without pruning
        self.warmup = trial.suggest_int("warmup", 0, 100, step=1)  # Number of epochs before starting pruning
        
        # BatchNorm hyperparameter
        self.use_batchnorm = trial.suggest_categorical("use_batchnorm", [True, False])
        
        self.epochs = args.max_epochs
        
        print(f"Trial {trial.number}: h={self.genotype_hiddens}, lr={self.lr:.2e}, drop={self.dropout:.2f}, act={activation_choice}, batch_size={int(2 ** self.batch_size_power)}, prune_freq={self.prune_frequency}, prune_pct={self.prune_percentage:.3f}, warmup={self.warmup}, batchnorm={self.use_batchnorm}")
        sys.stdout.flush()
    ...
    def train_model(self, trial):
        ...
        try:
            ...
            # Create GlobalPrunedFC_NN model with hyperparameters
            # Use trial number to create unique seed for each trial while maintaining reproducibility
            trial_seed = trial.number + args.seed
            # Model creation
            
            try:
                model = GlobalPrunedFC_NN(
                    input_dim=689,  # Fixed input dimension as required by GlobalPrunedFC_NN
            ...
            min_loss = None
            has_best = False  # True once we've written model_best.pt for this trial
            counter = 0
            ...
            for epoch in range(epochs):
                for i, (input_data, labels) in enumerate(cl_dataloader):
                    ...
                    if torch.any(torch.isnan(pred)) or torch.any(torch.isinf(pred)):
                        ...
                        print(f"Prediction stats: min={pred.min().item():.6f}, max={pred.max().item():.6f}")
                        continue
                    ...
                    if torch.isnan(mse_loss) or torch.isinf(mse_loss):
                        print(f"Warning: MSE loss is NaN/inf. Skipping batch.")
                        continue
                ...
                # Prune model every prune_frequency epochs, but only after warmup period
                if (self.prune_percentage > 0.0 and 
                    (epoch + 1) > self.warmup and 
                    (epoch + 1 - self.warmup) % self.prune_frequency == 0):
                    model.train()  # Set to train mode for pruning
                    model.prune(sparsity_level=self.prune_percentage)
                    print(f"Pruned model at epoch {epoch + 1} with {self.prune_percentage:.3f} sparsity level (warmup: {self.warmup} epochs)")
                    sys.stdout.flush()
                ...
                with torch.no_grad():
                    ...
                    # but only once the model has been pruned to its target sparsity.
                    # First qualifying epoch always saves; subsequent epochs save only on a strict improvement.
                    if model.is_fully_pruned_to_target():
                        if min_loss is None or (min_loss - new_val_loss).item() > 1e-4:
                            ...
                            has_best = True
                            print(f"Model saved at epoch {epoch} (fully pruned to target)")
            ...
            # Resolve the best checkpoint and bake pruning masks into a single
            # portable model_best.pt that contains plain `weight` parameters.
            best_path = f"{trial_dir}/model_best.pt"
            if has_best:
                print("\nLoading best model checkpoint and baking pruning masks...")
                model.load_state_dict(torch.load(best_path, map_location=device))
            elif model.is_fully_pruned_to_target():
                print("\nNo best checkpoint recorded; using final model state at target sparsity...")
            else:
                print("\nNo valid best model found, NeST sparsity target not achieved")
                return -1000.0

            for layer in model.NN:
                if isinstance(layer, torch.nn.Linear) and hasattr(layer, 'weight_mask'):
                    prune.remove(layer, 'weight')
            torch.save(model.state_dict(), best_path)
            print("Best model saved as model_best.pt (pruning masks removed)")

            # Reload from disk so evaluation runs on exactly the file that was saved.
            model.load_state_dict(torch.load(best_path, map_location=device))
            model.eval()
...
if __name__ == "__main__":
    hparam_trainer = OptunaGlobalPruneNNTrainer()
